...
export async function processEmail(email) {
  // Build unsubscribe context if sender was previously unsubscribed
  const unsubContext = email.previouslyUnsubscribed
    ? `\n\nIMPORTANT CONTEXT: The user has PREVIOUSLY UNSUBSCRIBED from this sender (${email.email}). This means they received another email despite unsubscribing. Factor this into your recommendation - suggest unsubscribing again or deleting.`
    : "";
...
Respond with this exact JSON structure:
{
  ...
  "suggest_unsubscribe": true or false,
  "smart_actions": [
    {
...
- Set suggest_unsubscribe to true for: newsletters, marketing emails, promotional content, automated notifications the user probably doesn't want${email.previouslyUnsubscribed ? "\n- Since the user PREVIOUSLY UNSUBSCRIBED from this sender, set suggest_unsubscribe to true" : ""}
...
- Be generous with smart_actions - if there is ANY date, event, deadline, or actionable item, include it`;
  ...
  try {
    ...
    return {
      ...
      suggestUnsubscribe: result.suggest_unsubscribe || false,
      smartActions: result.smart_actions || [],
    };
  } catch (err) {
    logError('ai:processEmails', 'AI processing failed', err);
    return {
      ...email,
      category: "Other",
      color: "#94a3b8",
